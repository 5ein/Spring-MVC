...
public class MongoDBTest2_update2 {
	...
	public static void main(String[] args) {
		...
		//update 할 조건은 json(document)로 만들어주세요.
		Document filter = new Document();
		filter.append("id", "friday");
		
		//update 할 내용은 bson(json)를 만들어주세요.
		...
		Bson setts = Updates.combine(list); //list를 bson으로 바꿔줘야함
		
		collection.updateMany(filter, setts); //두번째칸에는 json이어야함
		client.close();
